...
function CardArt({ art }: { art: (typeof data.cards)[number]["art"] }) {
  ...
  return (
    <span className="flex h-full w-full items-center justify-center rounded-full bg-cream-card text-navy-900">
      <Art className="h-[58%] w-[58%]" />
    </span>
  );
}
...
export function FocusSystemSection() {
  return (
    <section
      id="focus-system"
      className="app-screen-snap-target relative overflow-hidden bg-yellow-500 py-16 lg:pb-[4vw] lg:pt-[3.9vw]"
    >
      ...
      <div className="relative mx-auto w-full max-w-[93.25rem] px-5 md:px-8 lg:px-0">
        <div className="motion-reveal text-center">
          <h2 className="text-[1.95rem] font-black leading-[1.2] tracking-[-0.05em] text-navy-900 lg:text-[clamp(2.2rem,3.9vw,4.1rem)]">
            {data.headlineLead}
            <span className="text-red-500">{data.headlineAccent}</span>
            {data.headlineTail}
          </h2>
          <p className="mt-3 text-[0.95rem] text-navy-900/75 lg:mt-[0.8vw] lg:text-[clamp(0.9rem,1.25vw,1.32rem)]">
            {data.subhead}
          </p>
        </div>
        ...
        <ul className="mt-10 flex flex-col items-stretch gap-4 lg:mt-[3.4vw] lg:flex-row lg:items-stretch lg:gap-0">
          {data.cards.map((card) => (
            <li key={card.title} className="flex flex-col items-center lg:flex-1 lg:flex-row">
              <article className="flex w-full flex-1 flex-col items-center rounded-3xl bg-cream-card px-5 py-7 text-center shadow-[0_16px_38px_rgba(1,23,80,0.14)] lg:px-[1.2vw] lg:py-[2.5vw]">
                ...
                <span aria-hidden="true" className="mt-3 block h-0.5 w-14 bg-gold-600 lg:mt-[0.9vw]" />

                <span className="mt-5 block aspect-square w-[9.5rem] lg:mt-[2vw] lg:w-[12.8vw]">
                  <CardArt art={card.art} />
                </span>

                <p className="mt-5 text-[0.9rem] leading-[1.8] text-navy-900/80 lg:mt-[2vw] lg:text-[clamp(0.9rem,1.14vw,1.2rem)]">
              ...
              {card.connector ? (
                <span className="my-3 flex justify-center lg:my-0 lg:-mx-[1.8vw] lg:self-center">
